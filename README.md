...
This is a _very_ basic monitoring and documentation solution meant for simple home use. It will monitor the status of IP systems and services in a simple dashboard. Some basic service and device type definitions are supplied; however more can be added by altering the YAML configuration files.
...
- [Vault](#vault)
  ...
  - [Vault Host Matching](#vault-host-matching)
...
* vault - enables or disables the [KeePass vault integration](#vault)
...
While you certainly could, it is not a good idea to store authentication information within the [host documentation](#host-documentation) system. Trash Panda can read files generated by the [KeePassXC](https://keepassxc.org/) local password management system. This functionality is disabled by default but can be enabled in the [global config](#global-configuration).

This integration is not meant to expose all KeepPass information but will show username/password information. Entries should continue to be managed within the KeePass GUI or other KeePass management client.

### Using Vault

Once enabled, a Vault tab will appear on each host status page. You can unlock your KeePass Vault file through this tab, or through the configuration menu. Once unlocked the Vault page will show all entries in your KeePass vault. Individual host pages will show matching entries for that host using [tag matching](#vault-host-matching). The vault will remain unlocked for a period of one hour or until you lock it through the configuration menu.

### Vault Host Matching

Host status pages attempt to find matching host and service level entries within the Vault. These are matched using the following criteria:

Host entries are discovered through entry tagging. If you have a host named __My Web Server__, tag any entries in your Vault with a tag called __My Web Server__ to indicate that they should be matched with this host. The tag is irrelevant of any KeePass grouping you may have entries filed under. Trash Panda will show all entries matching the host tag under the Vault tab when viewing the host status page.

If an entry is matched with a Host through tagging, it can further be matched with a Service as well. If the entry name matches the service name a special unlock icon will be displayed in the services list. This is just a shortcut to quickly copy the password for the service without having to switch to the Vault tab.
...
* [pykeepass](https://github.com/libkeepass/pykeepass) - KeePassXC Python library
