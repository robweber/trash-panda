...
def async_command(self, host, command):
    progress = 10
    self.update_state(state='RUNNING', meta={'progress': progress, 'message': f"Preparing command on {host['name']}"})
    ...
    if(command_info is not None):
        progress = 50
        self.update_state(state='RUNNING', meta={'progress': progress, 'message': f"Running {command_info['name']} on {host['name']}"})
